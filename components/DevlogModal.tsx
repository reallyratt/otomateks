...
export const DevlogModal: React.FC = () => {
    ...
    return (
        <div className="text-sm text-brutal-text space-y-6">
            ...
            <div className="bg-brutal-surface border-4 border-brutal-border">
                <button 
                    onClick={() => setIsOpen(!isOpen)}
                    className="w-full font-black uppercase p-4 cursor-pointer hover:bg-brutal-bg transition-colors flex justify-between items-center"
                >
                    <div className="flex items-center gap-2">
                         <SlidersIcon className="w-5 h-5" />
                         <span>Dev Log History</span>
                    </div>
                    <span className={`border-2 border-brutal-border p-1 transition-colors ${isOpen ? 'bg-brutal-accent text-brutal-white' : ''}`}>
                        <svg className={`w-4 h-4 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7"></path></svg>
                    </span>
                </button>
                <div className={`grid transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr] border-t-4 border-brutal-border' : 'grid-rows-[0fr]'}`}>
                    <div className="overflow-hidden">
                        <div className="p-4 bg-brutal-bg space-y-6">
                            <UpdateItem 
                                version="1" 
                                title="Text Field" 
                                items={[
