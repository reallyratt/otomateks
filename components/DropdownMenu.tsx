...
export const DropdownMenu: React.FC<DropdownMenuProps> = ({ onTutorialClick, onSettingsClick, onDevlogClick, t }) => {
    ...
    const MenuButton: React.FC<{ onClick: () => void; children: React.ReactNode; icon: React.ReactNode }> = ({ onClick, children, icon }) => (
        <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold uppercase border-b-2 border-brutal-border last:border-b-0 hover:bg-brutal-accent hover:text-brutal-white transition-colors text-brutal-text"
        >
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`p-2 border-4 border-brutal-border transition-all duration-200 ${isOpen ? 'bg-brutal-border text-brutal-bg' : 'bg-brutal-surface text-brutal-text hover:bg-brutal-bg shadow-brutal active:translate-x-[2px] active:translate-y-[2px] active:shadow-none'}`}
                aria-haspopup="true"
            ...
            <div 
                className={`absolute right-0 mt-2 w-56 bg-brutal-surface border-4 border-brutal-border shadow-brutal-lg z-20 transition-all duration-200 ease-out origin-top-right
                    ${isOpen 
